...
class DatabaseFunctions:
    ...
    def merge(self, sql_query: str, delivered_con_string: str, delivered_database: str, delivered_schema: str, delivered_table: str,merge_column: str,merge_condition: str = "UPDATE"):
        if self._has_missing_args() or not delivered_database:
            logging.error('Por favor, forneça o database, schema e a tabela base da classe, além do banco de destino.')
            return 
        ...
        try:
            ...
            full_merge_query = f"""MERGE INTO {delivered_database}.{delivered_schema}.{delivered_table} USING ({sql_query}) 
            as source ON ({delivered_table}.{merge_column} = source.{merge_column})
            WHEN MATCHED THEN {merge_condition}
            WHEN NOT MATCHED THEN INSERT;
            FROM {delivered_database}.{delivered_schema}.{delivered_table}
            ORDER BY {merge_column}"""
            
            logging.info(f"Executando merge: {full_merge_query}")
            con_duckdb.execute(full_merge_query)
            logging.info("Merge executado com sucesso!")
            
        except Exception as e:
            logging.error(f"Falha na execução do merge com DuckDB: {e}")
            raise e 
        finally:
            con_duckdb.close()
    ...
    def incremental_upsert(self,
            ...
            Args:
                ...
                coluna_referencia_data (str, opcional): Nome da coluna na origem usada para calcular 
                    a data máxima do lote processado. O padrão é "dt_venda".
                coluna_merge (str): Nome da coluna usada para identificar registros duplicados durante o merge.
                condition_merge (str, opcional): Condição de merge a ser aplicada quando houver correspondência de registros. O padrao é update.
            Raises:
                Exception: Propaga qualquer erro ocorrido durante a inserção, garantindo que 
                    o fluxo seja interrompido e o checkpoint não seja atualizado indevidamente.
            """
            ...
            if nova_data_maxima is not None and str(nova_data_maxima) != 'NaT':
                try:
                    self.merge(
                        ...
                        delivered_schema=target_schema,
                        delivered_table=target_table,
                        merge_column=coluna_merge,
                        merge_condition=condition_merge
                    )
                    commit_checkpoint(
                        checkpoint_name=checkpoint_name,
                        nova_data_maxima=nova_data_maxima
